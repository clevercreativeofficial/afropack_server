...
if ($id <= 0) {
    $_SESSION['video'] = 'Invalid videos ID.';
...
if (!$video) {
    $_SESSION['video'] = 'Videos not found.';
    header('Location: ' . $url . 'admin/videos/');
    exit;
}
?>
    ...
    <main class="flex-1 overflow-y-auto py-6 px-3">
        ...
        <div id="videos" class="dashboard-section max-w-3xl mx-auto">
            <div class="flex flex-col md:flex-row items-center justify-between gap-4 mb-6">
                <h3 class="text-2xl font-bold text-accent-dark">Edit Video</h3>
                <button onclick="window.history.back()" class="bg-accent sm:w-auto w-full text-white px-4 py-2">
                    Back
                </button>
            </div>
            <form action="<?= $url ?>admin/videos/api/edit.php" method="POST" class="w-full bg-white space-y-4 py-6 px-4">
                <div class="grid grid-cols-1 gap-4">
                    ...
                    <div>
                        ...
                        <textarea name="description" class="outline-none w-full min-h-24 py-3 px-4 border focus:border-accent duration-300" placeholder="250 char max." required><?= $video['description'] ?></textarea>
                    </div>
                    <div>
                        ...
                        class="outline-none w-full py-3 px-4 border focus:border-accent duration-300"
                        placeholder="https://youtu.be/DlMAIYd7-J4" required>
                    </div>
                </div>
